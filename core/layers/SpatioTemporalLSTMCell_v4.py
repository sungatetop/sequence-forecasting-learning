...
class SpatioTemporalLSTMCell(nn.Module):
    ...
    def map_interval(self,et, dim):
        #论文中把间隔除于了平均间隔，是序列内的平均值，实际不要在此处操作，直接传入平均值
        T = 1.0 / torch.log(et + 2.7183) #论文中把间隔除于了平均间隔，此处输入的是已处理的
    ...
    def forward(self, x_t,e_t , h_t, c_t, m_t):
        ...
        #print("E_t",E_t.shape,D_ST.shape)
        D_ST_dis=E_t*D_ST #对短时记忆打折 #xue论文里合并在一起了
        #long term memory
        D_long=c_t-D_ST
        D_cor=D_ST_dis+D_long #D_cor=E_t*D_ST+c_t-D_ST

        c_new = f_t * D_cor + delta_c
