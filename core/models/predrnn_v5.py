...
import torch
import torch.nn as nn
from core.layers.SpatioTemporalLSTMCell_v4 import SpatioTemporalLSTMCell
...
class RNN(nn.Module):
    ...
    def forward(self, frames_tensor,distance_t, mask_true):
        ...
        d_last=distance_t[:,0,:] #因为我的数据里d都是与第一帧的距离，间隔需要做一下计算
        for t in range(self.configs.total_length - 1):
            ...
            if t==0:
                ...
                e_t=d_t-d_last
                d_last=d_t
            #print("e_t",e_t.shape,e_t)
            #把计算好的间隔信息送进去计算
            h_t[0], c_t[0], memory, delta_c, delta_m = self.cell_list[0](net, e_t, h_t[0], c_t[0], memory)
            ...
            for i in range(1, self.num_layers):
                h_t[i], c_t[i], memory, delta_c, delta_m = self.cell_list[i](h_t[i - 1], e_t , h_t[i], c_t[i], memory)
