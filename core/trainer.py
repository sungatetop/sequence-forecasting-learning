...
def train(model, ims, real_input_flag, configs, itr):
    ...
    record(configs.dataset_name+configs.model_name,current_time,itr,cost)
    if itr % configs.display_interval == 0:
        print(current_time, 'itr: ' + str(itr))
        print('training loss: ' + str(cost))
#注意数据传入的是与第一帧的距离
def train_with_interval(model,ims,distance,real_input_flag, configs, itr):
    cost = model.trainV3(ims,distance, real_input_flag)
    if configs.reverse_input:
        ims_rev = np.flip(ims, axis=1).copy()
        cost += model.trainV3(ims_rev,distance,real_input_flag)
        cost = cost / 2
    current_time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    record(configs.dataset_name+configs.model_name,current_time,itr,cost)
    if itr % configs.display_interval == 0:
...
def test(model, test_input_handle, configs, itr):
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        #添加一个测试
        if configs.model_name=="predrnn_v3":
            bs=test_ims.shape[0]
            seq_len=test_ims.shape[1]
            distance=np.linspace(0,seq_len-1,seq_len)
            b=distance.reshape(20,1)
            b=b[np.newaxis,:]
            batch_d=np.repeat(b,bs,axis=0)
            img_gen = model.testV3(test_dat,batch_d,real_input_flag)
        else:
