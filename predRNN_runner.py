...
parser.add_argument('--save_dir', type=str, default='checkpoints/mnist_predrnn1')
parser.add_argument('--gen_frm_dir', type=str, default='results/mnist_predrnn1')
...
def train_wrapper(model):
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        ims = train_input_handle.get_batch()#1个批次算一次迭代
        print("ims",ims.shape)
        ...
        #做一个测试对moving mnist数据集加入间隔信息
        if args.model_name=="predrnn_v3":
            bs=ims.shape[0]
            seq_len=ims.shape[1]
            distance=np.linspace(0,seq_len-1,seq_len)
            b=distance.reshape(20,1)
            b=b[np.newaxis,:]
            batch_d=np.repeat(b,bs,axis=0)
            trainer.train_with_interval(model, ims,batch_d,real_input_flag, args, itr)
        else:
